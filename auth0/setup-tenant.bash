...
API_NAME=candidate-composite
API_URL=https://localhost:8443/candidate-composite
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST -H "Content-Type: application/json" -d "{\"name\": \"$API_NAME\", \"identifier\": \"$API_URL\", \"scopes\": [{ \"value\": \"candidate:read\", \"description\": \"Read candidate information\"}, {\"value\": \"candidate:write\", \"description\": \"Update candidate information\"}]}" https://$TENANT/api/v2/resource-servers | jq .
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$READER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"candidate:read\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
...
then
  ...
  curl -s -H "Authorization: Bearer $AT" -X POST  -H "Content-Type: application/json" -d "{\"client_id\":\"$WRITER_CLIENT_ID\",\"audience\":\"$API_URL\",\"scope\":[\"candidate:read\",\"candidate:write\"]}" https://$TENANT/api/v2/client-grants | jq .
  echo
fi
